...
\mf~will write a version 2 binary grid file when the discretization supports the user input option CRS and that option is defined.  If the discretization supports the CRS option but it is not defined in the discretization OPTIONS block, the version 1 file will be generated.  Version 2 binary grid files contain an additional definition string and data record associated with the CRS input.  An example for the DIS grid follows.
...
\noindent Definition 15: \texttt{`IDOMAIN INTEGER NDIM 1 ncells'} {\color{red} \footnotesize{CHARACTER(LEN=LENTXT)}} \\
\noindent Definition 16: \texttt{`ICELLTYPE INTEGER NDIM 1 ncells'} {\color{red} \footnotesize{CHARACTER(LEN=LENTXT)}} \\
\noindent Definition 17: \texttt{`CRS CHARACTER NDIM 1 len\_trim'} {\color{red} \footnotesize{CHARACTER(LEN=LENTXT)}} \\
...
\noindent Record 15: \texttt{(IDOMAIN(J),J=1,NCELLS)} {\color{red} \footnotesize{INTEGER ARRAY SIZE(NCELLS)}} \\
\noindent Record 16: \texttt{(ICELLTYPE(J),J=1,NCELLS)} {\color{red} \footnotesize{INTEGER ARRAY SIZE(NCELLS)}} \\
\noindent Record 17: \texttt{(CRS(J),J=1,LEN\_TRIM)} {\color{red} \footnotesize{CHARACTER ARRAY SIZE(LEN\_TRIM)}} \\
