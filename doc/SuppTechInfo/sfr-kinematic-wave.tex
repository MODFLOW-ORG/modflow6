...
The one-dimensional Saint-Venant equations for open-channel flow combines the continuity equation and the momentum equation. The momentum equation includes terms representing local acceleration, advective acceleration, pressure gradient, gravity, and friction. The kinematic wave approximation retains only the gravity and friction terms in the momentum equation, which is equivalent to assuming that the energy grade line is parallel to the channel bed slope. Under this approximation, the discharge $Q$ ($L^3 T^{-1}$) is uniquely determined by the depth of flow and the channel geometry, and the governing equation reduces to the continuity equation alone \citep{LighthillWhitham1955, PonceSimons1977, PonceLiSimons1978}
...
The kinematic wave celerity $c$ ($L T^{-1}$) is the speed at which a perturbation (a flood peak) propagates downstream. It is defined as the derivative of discharge with respect to cross-sectional area
...
\noindent The updated discharge is floored at $10^{-30}$~m$^3$\,s$^{-1}$ to maintain a positive definite flow. Iteration continues until
...
The TVD scheme bypasses iterative residual-solving for the continuity equation by advancing the reach state directly through an explicit volume balance. At each time step the scheme operates as follows.
...
The \texttt{ATS\_COURANT} value sets the target Courant number for the Adaptive Time Stepping (ATS) module in \mf. At the start of each time step, the SFR Package estimates the Courant number from the kinematic wave celerity (eq.~\ref{eqn:sfr-courant}) and submits a recommended time step $\Delta t = C_r^\mathrm{target}\,\Delta x / c$ to the ATS controller. The controller may select a shorter step if other packages or the outer nonlinear solver impose tighter constraints; the TVD scheme then uses whatever step the ATS controller provides and delegates to the standard Newton-Raphson solver when the resulting Courant number exceeds unity. Setting \texttt{ATS\_COURANT} = 1 targets $C_r = 1$, which minimizes both numerical diffusion and the van Leer anti-diffusion correction; smaller values are appropriate when the hydrograph must be resolved more finely in time.

At the end of each simulation, a \texttt{COURANT NUMBER FOR EACH REACH} table is written to the GWF model listing file when \texttt{ATS\_COURANT} is specified. The table reports the minimum, maximum, and time-averaged Courant number for every reach, providing a direct check on whether the ATS-controlled time steps kept $C_r$ within the intended range.
...
\begin{itemize}
  ...
  \item \textit{Explicit stability.} The TVD scheme is explicit and conditionally stable. When $C_r > 1$, the scheme automatically delegates to the implicit Newton-Raphson solver, so occasional Courant exceedances (e.g., during high-flow events) are handled gracefully. Sustained $C_r \gg 1$ will cause frequent delegation and may degrade solution quality; the ATS controller should be tuned to keep $C_r$ below unity for most steps.
...
\noindent where the base flow is $Q_0 = 5$ m$^3$s$^{-1}$ and the wave period is $T_\mathrm{wave} = 10$ times the single-reach travel time ($T$). The wave celerity at base flow is $c_0 \approx 1.18$ m$\,$s$^{-1}$, giving a travel time of $T = \Delta x / c_0 \approx 423$ s per reach.
...
Figure~\ref{fig:sfr-kinwave-tvd} compares the standard and TVD schemes against the exact solution at $C_r = 0.5$ (panel~\textit{A}) and $C_r = 0.75$ (panel~\textit{B}). At both Courant numbers, the standard scheme diffuses the wave, reducing the peak amplitude and spreading the hydrograph. The TVD scheme, with the van Leer anti-diffusion correction active at the downstream reach, closely tracks the exact solution: the peak amplitude and wave shape are well-recovered even at $C_r = 0.5$, where the standard scheme produces marked attenuation. This demonstrates that the TVD option can provide accurate flood-wave routing at time steps significantly shorter than the optimal $C_r \approx 1$ value required by the standard scheme.

\begin{figure}[!ht]
	\begin{center}
	\includegraphics{./Figures/SFRKinematicWaveTVDRouting.pdf}
	\caption[Graphs comparing the \mf standard kinematic-wave and TVD schemes against the exact solution at Courant numbers of 0.5 and 0.75 for a two-reach chain]{Graphs comparing the \mf standard kinematic-wave scheme ($\theta = 1$) and the TVD scheme against the exact kinematic-wave solution for the downstream reach of a two-reach \mf model. Each reach has $\Delta x = 500$ m, $B = 10$ m, $S_0 = 0.001$, $n = 0.03$ s$\,$m$^{-1/3}$, base flow $Q_0 = 5$ m$^3$s$^{-1}$, and the same sinusoidal upstream inflow (eq.~\ref{eqn:sfr-example-qin}). The exact downstream solution is the upstream hydrograph delayed by two travel times. Time is normalized by the single-reach travel time $T \approx 423$ s. \textit{A}, $C_r = 0.5$; \textit{B}, $C_r = 0.75$. At both Courant numbers the standard scheme attenuates the peak, while the TVD scheme closely matches the exact solution}
...
\begin{figure}[!ht]
	...
	\caption[Graphs comparing the per-step SFR budget percent discrepancy of the standard kinematic-wave and TVD schemes at Courant numbers of 0.5 and 0.75]{Graphs comparing the per-step SFR package budget percent discrepancy of the \mf standard kinematic-wave scheme ($\theta = 1$) and the TVD scheme for the same two-reach \mf model shown in figure~\ref{fig:sfr-kinwave-tvd}. Time is normalized by the single-reach travel time $T \approx 423$ s. The discrepancy axis is logarithmic; TVD values at zero are plotted at the floor of the axis. \textit{A}, $C_r = 0.5$; \textit{B}, $C_r = 0.75$. The TVD scheme closes to machine precision at every step; the standard scheme produces discrepancies approaching 1 percent, with the largest values when the flow gradient is steepest}
