...
\noindent where the base flow is $Q_0 = 5$ m$^3$s$^{-1}$ and the wave period is $T_\mathrm{wave} = 10$ times the travel time ($T$). The wave celerity at base flow is $c_0 \approx 1.18$ m$\,$s$^{-1}$, giving a travel time of $T = \Delta x / c_0 \approx 423$ s.
...
Because individual reaches in a network can differ markedly in length and slope, the Courant number will vary from reach to reach for a given time step. The Courant number summary table written to the listing file at the end of each simulation (see \nameref{sec:sfr-time-step}) reports the simulation-wide minimum, maximum, and mean $C_r$ for every reach, making it straightforward to identify which reaches have the largest or smallest Courant numbers and whether any reach consistently falls outside the acceptable range. Once the Courant field is understood, the time step can be adjusted to the largest value that keeps $C_r$ within the acceptable range for the most sensitive reaches. Subdividing unusually long reaches into shorter segments, or merging very short adjacent reaches, can help to equalize Courant numbers across the network and allow a longer global time step.
